...
def SSC_test():
    ...
    d1 = 27
    d2 = 29  # d1 and d2: dimension of subspace 1 and 2
    N1 = 500
    N2 = 500  # N1 and N2: number of points in subspace 1 and 2
    # Generating N1 points in a d1 dim. subspace
    X1 = np.matmul(np.random.randn(D, d1), np.random.randn(d1, N1))
    # Generating N2 points in a d2 dim. subspace
    X2 = np.matmul(np.random.randn(D, d2), np.random.randn(d2, N2))
    ...
    yale_data, label = data_helper.load_data()
    X = yale_data.reshape([yale_data.shape[0], -1])
    X = X.T
    s = label
